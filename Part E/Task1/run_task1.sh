...
run_sim() {
    ...
    echo "=========================================================="
    echo "Running ${WORKLOAD} - ${NAME} (Size: ${SIZE}, Latency: ${LATENCY} cycles)"
    echo "=========================================================="

    $GEM5 --outdir=$OUTDIR $SE_SCRIPT \
      --cpu-type=O3CPU --caches --l2cache --num-l2caches=1 \
...
echo ""
echo "=========================================================="
echo "                   TASK 1 RESULTS SUMMARY                "
echo "=========================================================="
printf "%-10s | %-10s | %-12s | %-8s | %-12s\n" "Workload" "L2 Type" "simSeconds" "IPC" "L2 Miss Rate"
echo "----------------------------------------------------------"

for W in bfs sssp; do
  for T in sram mram; do
    DIR="m5out_${W}_${T}"
    SIM_SEC=$(grep "simSeconds" $DIR/stats.txt 2>/dev/null | awk '{print $2}')
    IPC=$(grep "system.cpu.ipc" $DIR/stats.txt 2>/dev/null | awk '{print $2}')
    MISS=$(grep "system.l2.overallMissRate::total" $DIR/stats.txt 2>/dev/null | awk '{print $2}')
    printf "%-10s | %-10s | %-12s | %-8s | %-12s\n" "$W" "$T" "$SIM_SEC" "$MISS" "$IPC"
  done
done
echo "=========================================================="
